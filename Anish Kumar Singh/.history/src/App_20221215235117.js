...
// import DAnimation from "./components/DAnimation/App";
...
function App() {
  ...
  return (
    <Router>
      ...
      <div ref={codeforces}>{!ShowContact && <Codeforces mode={mode} />}</div>
      <div>
        <Routes>
          <Route
            exact
            path="/contact"
            SetShowContact(true0;)
            element={<Contact mode={mode} Handle={Handle} />}
          />
        </Routes>
      </div>
      {!ShowContact && <Footer mode={mode} />}
    </Router>
  );
}
export default App;
